...
class PP_Social_Login_Process
{
     ...
     */
    public function __construct($hybrid_profile_data)
    {
        ...
        if (isset($login_response) && is_wp_error($login_response)) {
            pp_login_wp_errors('failed_reg_login', $login_response->get_error_message());
        } elseif (isset($register) && is_wp_error($register)) {
            pp_login_wp_errors('failed_reg_login', $register->get_error_message());
        }
    }
    ...
    public function construct_username($hybrid_profile_data)
    {
        if ( ! empty($hybrid_profile_data->displayName)) {
            $username = sanitize_text_field($hybrid_profile_data->displayName);
    ...
    public function construct_email($hybrid_profile_data)
    {
        if (empty($hybrid_profile_data->emailVerified) && empty($hybrid_profile_data->email)) {
            $val = $this->username . '@' . esc_attr($_GET['pp_social_login']) . '.com';
        } else {
            $val = ! empty($hybrid_profile_data->emailVerified) ? $hybrid_profile_data->emailVerified : $hybrid_profile_data->email;
        }
    ...
    public function copy_social_avatar_locally($avatar_url)
    {
        ...
        $fileLocationAndPath = AVATAR_UPLOAD_DIR . "$filename.$file_ext";
    ...
    public function register_user()
    {
        $args = array(
            ...
            'role'        => apply_filters('pp_social_signup_role', get_option('default_role'))
        );

        do_action('pp_before_social_signup', $args, $_GET['pp_social_login']);

        $user_id = wp_insert_user(apply_filters('pp_social_user_signup_args', $args));

        if (is_int($user_id)) {

            wp_new_user_notification($user_id, null, 'admin');

            add_user_meta($user_id, '_pp_signup_via', sanitize_text_field($_GET['pp_social_login']));

            ProfilePress_Registration_Auth::send_welcome_email(
                null,
                $this->email,
                $this->username,
                null,
            ...
            if (self::is_moderation_enabled()) {
                ...
                if (ProfilePress_Registration_Auth::moderation_is_active()) {
            ...
            do_action('pp_after_social_signup', $args, $user_id, $_GET['pp_social_login']);
        }
    ...
    public function login_user($identifier = '', $registration_flag = false)
    {
        ...
        if (ProfilePress_User_Moderation_Admin::moderation_is_active()) {
            $account_active = ProfilePress_User_Moderation_Admin::login_authentication('', $username, '');

            if (is_wp_error($account_active)) {
                return $account_active;
            }
        }

        do_action('pp_before_social_login', $user_id, $this->profile_data);
        ...
        $social_login_redirect = apply_filters('pp_social_login_redirect', pp_login_redirect(), esc_html($_GET['pp_social_login']), $user_id, $registration_flag);

        wp_redirect($social_login_redirect);
        exit;
    }
    ...
    public function register_and_login()
    {
        ...
        if (is_int($registered_id)) {
            ...
            do_action('pp_after_social_registration', $registered_id, $this->profile_data);
        ...
        } elseif (is_wp_error($registered_id)) {
    ...
    {
        return apply_filters('enable_moderation_social_login', ProfilePress_Registration_Auth::moderation_is_active());
    }
}